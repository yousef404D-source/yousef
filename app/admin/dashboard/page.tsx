  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#050816] flex flex-col items-center justify-center text-white gap-4">
        ...
        <p className="text-sm font-medium text-slate-400">جاري فحص صلاحيات المسؤول السيبراني...</p>
      </div>
    );
  }
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto max-w-7xl mx-auto w-full">
        <div className="mb-8 relative overflow-hidden bg-gradient-to-l from-indigo-950/40 via-[#0a0f24] to-[#050816] border border-slate-800/60 rounded-3xl p-6 md:p-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-mono mb-3">
            <Sparkles className="w-3.5 h-3.5 animate-pulse" /> PRODUCTION LIVE
          </div>
          <h2 className="text-2xl md:text-3xl font-extrabold text-white mb-2">محرك توليد المواقع الذكي V2.5</h2>
          <p className="text-slate-400 text-sm max-w-2xl leading-relaxed">اكتب وصف فكرتك، وسيقوم نظام هوية الذكاء الاصطناعي بصياغة كود تفاعلي نقي وتوليد صفحة معاينة حية فوراً.</p>
        </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-[#0a0f24] border border-slate-800/80 rounded-3xl p-6">
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-800/60">
                <div className="flex items-center gap-3">
                  <div className={`p-2.5 rounded-xl transition-all ${isGenerating ? 'bg-amber-500/10 border border-amber-500/30' : isBotTyping ? 'bg-indigo-500/10 border border-indigo-500/30' : 'bg-slate-800'}`}>
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white">مساعد الإنشاء الذكي</h3>
                    <p className="text-[11px] text-slate-400">{isGenerating ? "جاري التوليد والتشفير الفوري الحقيقي..." : isBotTyping ? "الروبوت يستمع لمتطلباتك..." : "جاهز لاستلام فكرتك الجديدة"}</p>
                  </div>
                </div>
              </div>

              {error && (
                <div className="mb-4 p-4 bg-red-950/20 border border-red-500/30 text-red-400 rounded-xl text-xs flex gap-2">
              ...
              <form onSubmit={handleGenerateWebsite} className="space-y-4">
                <textarea 
                  ...
                  placeholder="مثال: موقع لمطعم برجر فاخر، يحتوي هيدر داكن، كروت للوجبات مع السعر، وقسم لآراء العملاء..." 
                  disabled={isGenerating} 
                  rows={5} 
                  className="w-full bg-[#050816] border border-slate-800 focus:border-indigo-500/60 rounded-2xl p-4 text-sm text-white placeholder-slate-600 focus:outline-none transition disabled:opacity-50 resize-none" 
                />
                <button 
                  type="submit" 
                  disabled={isGenerating || !prompt.trim()} 
                  className="w-full py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium rounded-2xl shadow-lg shadow-indigo-600/20 transition disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {isGenerating ? <><Loader2 className="w-5 h-5 animate-spin" /> <span>جاري البناء السحري والـ Deploy...</span></> : <><Sparkles className="w-4 h-4" /> <span>أطلق العنان وابنِ الموقع الآن</span></>}
                </button>
              </form>
            </div>

            {generatedUrl && (
              <div className="bg-[#0a0f24] border border-emerald-500/20 rounded-3xl p-6 bg-gradient-to-br from-emerald-950/10 to-[#0a0f24] space-y-4">
                <div className="flex items-center gap-3 text-emerald-400">
                  <CheckCircle className="w-5 h-5" />
                  <h4 className="text-sm font-bold">تم الانتهاء والرفع بنجاح الحقيقي!</h4>
                </div>
