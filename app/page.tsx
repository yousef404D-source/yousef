...
// --- خيارات الأسئلة التفاعلية (مطابقة للصورة تماماً) ---
...
export default function UltimateBuilderPage() {
  ...
  const [responsiveView, setResponsiveView] = useState<"full" | "mobile">("full"); 
      ...
      <div class="min-h-screen bg-[#0b0c10] flex items-center justify-center p-4 font-sans text-slate-100">
        <div class="w-full max-w-md bg-[#12141c] border border-cyan-500/20 rounded-3xl p-8 shadow-2xl relative">
          <div class="text-center mb-6">
            ...
            <p class="text-xs text-slate-500 mt-1">يرجى إدخال رمز الوصول للمتابعة وتفعيل لوحة العمل</p>
          </div>
  ...
  return (
    <div class="w-full h-screen min-h-screen bg-[#0a0b0d] text-slate-100 flex flex-col font-sans overflow-hidden">
      ...
      <header class="h-16 border-b border-slate-900 bg-[#0d0e12]/90 backdrop-blur px-6 flex justify-between items-center shrink-0 z-50">
        <div class="flex items-center gap-3">
          <div class={`w-9 h-9 rounded-xl flex items-center justify-center border transition-all duration-300 ${
          ...
          <div>
            ...
            <p class="text-[10px] text-slate-500">HYBRID INTERACTIVE LINK ACTIVE</p>
          </div>
        </div>

        {/* أزرار المود الذكي (محرك بناء / محادثة فائقة) */}
        <div class="flex bg-[#12141a] p-1 rounded-xl border border-slate-800">
          <button 
      ...
      <div class="flex-1 flex overflow-hidden w-full h-[calc(100vh-64px)]">
        
        {/* اليسار: المعاينة الحية المستقلة */}
        <section class="w-1/2 h-full bg-[#0e1014] p-4 flex flex-col border-r border-slate-900/60 overflow-hidden">
          <div class="flex justify-between items-center mb-3 shrink-0">
            <div class="flex items-center gap-1.5 text-slate-400 text-[11px] font-bold tracking-wider">
              <Globe class="w-3.5 h-3.5 text-cyan-400" /> DESKTOP SANDBOX OUTPUT
            </div>
            <div class="flex items-center gap-2">
              ...
              <button 
                ...
                class="bg-gradient-to-r from-emerald-500 to-teal-600 text-[#0b0c10] px-3.5 py-1.5 rounded-xl text-[11px] font-black hover:opacity-90 transition-all flex items-center gap-1"
              >
                {isDeploying ? <RefreshCw class="w-3 h-3 animate-spin" /> : "🚀 نشر الموقع / Deploy"}
              </button>
            </div>
          </div>

          <div class="flex-1 bg-white rounded-2xl overflow-hidden flex items-center justify-center relative">
            <iframe 
              srcDoc={previewCode}
        ...
        {/* اليمين: الشات النظيف المتناسق بالكامل */}
        <section class="w-1/2 h-full flex flex-col bg-[#0a0b0d] p-4 overflow-hidden justify-between">
          
          {/* حاوية الشات العلوية */}
          <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
            
            {/* أزرار التجاوب (Full Stack / Mobile App) مباشرة فوق شاشة المحادثة */}
            <div class="flex bg-[#12141a] p-1 rounded-xl border border-slate-800 self-start mb-3 shrink-0">
              ...
              >
                <Monitor class="w-3.5 h-3.5" /> Full Stack App
              </button>
              <button 
              ...
              >
                <Smartphone class="w-3.5 h-3.5" /> Mobile App
              </button>
            </div>

            {/* صندوق الرسائل الممتد */}
            <div class="flex-1 overflow-y-auto space-y-4 pr-1 mb-4 custom-scrollbar">
              {messages.map((msg, index) => (
                <div key={index} class={`flex flex-col ${msg.sender === "user" ? "items-end text-right" : "items-start text-left"}`}>
                  <div class={`max-w-[85%] rounded-2xl p-3.5 text-xs leading-relaxed shadow-md ${
                    msg.sender === "user" 
                      ? "bg-slate-800 text-slate-100 rounded-tr-none" 
                      : "bg-[#12151c] border border-slate-800 text-slate-300 rounded-tl-none"
                  }`}>
                    {msg.image && <img src={msg.image} alt="Sketch input" class="w-full max-h-32 object-cover rounded-xl mb-2 border border-slate-700" />}
              ...
              {showWizard && (
                <div class="bg-[#11141b] border border-slate-800/80 rounded-2xl p-5 shadow-2xl animate-fade-in my-2 text-left">
                  <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">
                    {QUESTIONNAIRE_STEPS[currentStepIndex].title}
                  </div>
                  <h3 class="text-xs font-bold text-slate-200 mb-4 leading-normal">
                        ...
                        <div key={opt.id} class="flex flex-col">
                          <label class={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                            ...
                            <input 
                              ...
                              class="rounded border-slate-700 text-cyan-400 focus:ring-0 bg-[#0b0c10]"
                            />
                          ...
                          {opt.isCustom && isChecked && (
                            <div class="mt-2 pl-4">
                              <textarea
                                ...
                                class="w-full bg-[#0b0c10] border border-slate-800 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-cyan-400 min-h-[55px]"
                              />
                            </div>
                          )}
                        </div>
                  ...
                  <div class="flex justify-between items-center border-t border-slate-800/60 pt-3.5">
                    <span class="text-[11px] text-slate-500">
                      &lt; Question {currentStepIndex + 1} of {QUESTIONNAIRE_STEPS.length} &gt;
                    </span>
                    <div class="flex gap-2">
                      <button type="button" onClick={handleAutoAnswer} class="bg-slate-800 text-slate-300 text-[11px] font-bold px-3 py-1.5 rounded-xl hover:bg-slate-700">
                        Auto-answer
                      </button>
                      <button type="button" onClick={handleNextStep} class="bg-cyan-400 text-[#0a0b0d] text-[11px] font-black px-4 py-1.5 rounded-xl hover:bg-cyan-300">
          ...
          <form onSubmit={handleSendMessage} class="bg-[#111317] border border-slate-800 rounded-2xl p-3 flex flex-col gap-2 shrink-0 shadow-lg">
            <textarea
              value={inputMessage}
              ...
              rows={2}
              class="w-full bg-transparent text-xs text-slate-200 placeholder-slate-600 resize-none focus:outline-none"
            />
            ...
            <div class="flex justify-between items-center border-t border-slate-800/60 pt-2">
              <div class="flex items-center gap-1">
                <button 
                  ...
                  class={`p-2 rounded-xl transition ${uploadedImage ? "bg-cyan-950 text-cyan-400" : "text-slate-500 hover:bg-slate-800"}`}
                >
                ...
                <button 
                  ...
                  class={`p-2 rounded-xl transition ${isRecording ? "bg-red-950 text-red-400" : "text-slate-500 hover:bg-slate-800"}`}
                >
                  {isRecording ? <MicOff class="w-4 h-4" /> : <Mic class="w-4 h-4" />}
                </button>
                {uploadedImage && <span class="text-[9px] bg-cyan-950 text-cyan-400 px-2 py-0.5 rounded border border-cyan-900">Image Attached</span>}
              </div>

              <button type="submit" class="bg-slate-800 text-slate-300 p-2 rounded-xl hover:bg-cyan-400 hover:text-[#0b0c10] transition-all">
