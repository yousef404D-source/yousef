...
export async function routeRequest(
  ...
  const raw = await generateText(FAST_MODEL, {
    system:
      'Classify the latest user message for a website-building AI called Nova. Respond ONLY with compact JSON: {"mode":"CHAT"|"BUILD","scope":"FULL"|"TARGETED","needsVisual":true|false,"visualPrompt":"<image prompt or empty>","note":"<5 words>"}. CHAT = question, conversation, explanation, thanks, asking who Nova is / what it can do, any technical question not asking to build/change a site (including questions about programming languages other than the website itself). BUILD = asking to create or modify a website/app. scope TARGETED = a small specific change to something that already exists (e.g. "make the button blue"). scope FULL = building from scratch or a broad redesign. If no site exists yet, BUILD is always FULL. needsVisual = true ONLY when a real photographic/illustrated hero or background image would clearly elevate this specific site (e.g. a travel/restaurant/product hero shot) — false for anything a gradient, icon set, or CSS pattern already covers, and always false for TARGETED edits. When true, visualPrompt is a short descriptive image-generation prompt matching the site\'s subject and tone. The message may be in Arabic, English, or mixed — classify by meaning, not language.',
    messages: [
      {
        role: "user",
